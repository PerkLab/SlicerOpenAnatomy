...
void vtkGLTFExporter::WriteAnActor(vtkActor *Actor)
{
  ...
  ac.componentType = TINYGLTF_PARAMETER_TYPE_FLOAT;
  ...
  aPrimitive.attributes.insert(std::make_pair("POSITION", this->Model.accessors.size() ));
  ...
  for (polygons->InitTraversal(); polygons->GetNextCell(npts, indx); )
  {
    for (int j = 0; j < npts; ++j)
    {
	  ...
	  std::vector<unsigned char> intToBytes(int intValue);
  ...
  ac2.componentType = TINYGLTF_PARAMETER_TYPE_UNSIGNED_INT;
